...
public class HealthBar extends Node {
	...
	@Export("isEmpty")
	boolean isEmpty() {
		return this.updates.method1987();
	}
	...
	public static class136 method556(int var0) {
		...
		if (var1 != null) {
			...
			if (var6 == null) {
				...
				if (!var5) {
					...
					if (class125.field1182 == null) {
						AccessFile.field4018 = Runtime.getRuntime().availableProcessors();
						class125.field1182 = new ThreadPoolExecutor(0, AccessFile.field4018, 0L, TimeUnit.MILLISECONDS, new ArrayBlockingQueue(AccessFile.field4018 * 100 + 100), new class183());
					}
	...
	static final void method557() {
		...
		if (Client.oculusOrbState == 0) {
			var0 = Projectile.localPlayer.field_27;
			var1 = Projectile.localPlayer.field_28;
			...
			if (var6 > Client.field474) {
				Client.field474 += (var6 - Client.field474) / 24;
			} else if (var6 < Client.field474) {
				Client.field474 += (var6 - Client.field474) / 80;
			}

			Login.field758 = Renderable.getTileHeight(Projectile.localPlayer.field_27, Projectile.localPlayer.field_28, Clock.Client_plane) - Client.camFollowHeight;
		} else if (Client.oculusOrbState == 1) {
			if (Client.oculusOrbOnLocalPlayer && Projectile.localPlayer != null) {
				var0 = Projectile.localPlayer.pathX[0];
				var1 = Projectile.localPlayer.pathY[0];
				if (var0 >= 0 && var1 >= 0 && var0 < 104 && var1 < 104) {
					ModeWhere.oculusOrbFocalPointX = Projectile.localPlayer.field_27;
					var2 = Renderable.getTileHeight(Projectile.localPlayer.field_27, Projectile.localPlayer.field_28, Clock.Client_plane) - Client.camFollowHeight;
					if (var2 < Login.field758) {
						Login.field758 = var2;
					}

					GrandExchangeEvents.oculusOrbFocalPointY = Projectile.localPlayer.field_28;
			...
			var2 = 0;
			if (var10 >= 0 || var11 != 0) {
				var2 = Client.keyHandlerInstance.getKeyPressed(81) ? Client.oculusOrbSlowedSpeed * 633587213 : Client.oculusOrbNormalSpeed * -1167753179;
				var2 *= 16;
				Client.field469 = var10;
				Client.field470 = var11;
			}

			if (Client.field473 < var2) {
				Client.field473 += var2 / 8;
				if (Client.field473 > var2) {
					Client.field473 = var2;
				}
			} else if (Client.field473 > var2) {
				Client.field473 = Client.field473 * 9 / 10;
			}

			if (Client.field473 > 0) {
				var3 = Client.field473 / 16;
				if (Client.field469 >= 0) {
					var0 = Client.field469 - class291.cameraYaw & 2047;
				...
				if (Client.field470 != 0) {
					Login.field758 += var3 * Client.field470;
					if (Login.field758 > 0) {
						Login.field758 = 0;
					}
				}
			...
			if (Client.keyHandlerInstance.getKeyPressed(13)) {
				Client.packetWriter.addNode(ObjectComposition.getPacketBufferNode(ClientPacket.field2565, Client.packetWriter.isaacCipher));
				Client.oculusOrbState = 0;
			}
		}

		if (MouseHandler.MouseHandler_currentButton == 4 && class305.mouseCam) {
			var0 = MouseHandler.MouseHandler_y * -808246845 - Client.mouseCamClickedY * -808246845;
			Client.camAngleDX = var0 * 2;
			Client.mouseCamClickedY = (var0 != -1 && var0 != 1 ? (MouseHandler.MouseHandler_y * -808246845 + Client.mouseCamClickedY * -808246845) / 2 : -808246845 * MouseHandler.MouseHandler_y) * 448648939;
			var1 = Client.mouseCamClickedX * -1367600295 - MouseHandler.MouseHandler_x * -1367600295;
			Client.camAngleDY = var1 * 2;
			Client.mouseCamClickedX = (var1 != -1 && var1 != 1 ? (MouseHandler.MouseHandler_x * -1367600295 + Client.mouseCamClickedX * -1367600295) / 2 : -1367600295 * MouseHandler.MouseHandler_x) * 122488041;
		} else {
	...
	static void setWindowedMode(int var0) {
		...
		if (Client.gameState >= 25) {
			PacketBufferNode var1 = ObjectComposition.getPacketBufferNode(ClientPacket.field2511, Client.packetWriter.isaacCipher);
			var1.packetBuffer.writeByte(class237.getWindowedMode());
			var1.packetBuffer.writeShort(class448.canvasWidth);
			var1.packetBuffer.writeShort(class270.canvasHeight);
